...
function completeDocumentInserted(ret_obj) {
  ...
  if (
    current_category_srl &&
    category_srl &&
    current_category_srl == category_srl
  ) {
...
function completeInsertComment(ret_obj) {
  ...
  if (ret_obj.redirect_url) {
    ...
    if (comment_srl)
      url =
        url.setQuery("comment_srl", comment_srl) + "#comment_" + comment_srl;
    redirect(url);
  }
}
...
function doChangeCategory() {
  ...
  location.href = decodeURI(current_url)
    .setQuery("category", category_srl)
    .setQuery("page", "");
}
...
jQuery(function ($) {
  $(document.body).click(function (e) {
    var t = $(e.target),
      ...
      params = {};
    if (
      t.parents(".layer_voted_member").length === 0 &&
      !t.is(".layer_voted_member")
    ) {
...
/* Original file: files/cache/assets/minified/modules.editor.tpl.js.editor_common.min.js */

var editorMode = [];
var editorAutoSaveObj = {
  fo_obj: null,
  editor_sequence: 0,
  title: "",
  content: "",
...
function editorGetContent(editor_sequence) {
  ...
  var reg_pattern = new RegExp(
    request_uri.replace(/\//g, "\\/") +
      "(files|common|modules|layouts|widgets)",
    "ig",
  );
...
function editorEnableAutoSave(fo_obj, editor_sequence, callback) {
  ...
  editorEnableAutoSave.timer = setTimeout(function () {
    _editorAutoSave(!1, callback);
  }, 50000);
}
...
function _editorAutoSave(exe, callback) {
  ...
  if (!exe) {
    clearTimeout(editorEnableAutoSave.timer);
    editorEnableAutoSave.timer = setTimeout(function () {
      _editorAutoSave(exe, callback);
    }, 50000);
  }
  if (editorAutoSaveObj.locked === !0) return;
  ...
  if (
    title != editorAutoSaveObj.title ||
    content != editorAutoSaveObj.content ||
    exe
  ) {
    var params,
      oDate = new Date();
    ...
    jQuery("#editor_autosaved_message_" + editor_sequence)
      .text(oDate.toTimeString().substring(0, 5) + " " + auto_saved_msg)
      .show(300);
    editorAutoSaveObj.locked = !0;
    show_waiting_message = !1;
    exec_xml("editor", "procEditorSaveDoc", params, function () {
      var arg = jQuery.extend({}, params, { auto_saved_msg: auto_saved_msg });
      editorAutoSaveObj.locked = !1;
...
function editorGetIFrame(editor_sequence) {
  if (
    editorRelKeys !== undefined &&
    editorRelKeys[editor_sequence] !== undefined &&
    editorRelKeys[editor_sequence].editor !== undefined
  ) {
...
function openComponent(component_name, editor_sequence, manual_url) {
  ...
  var popup_url =
    request_uri +
    "?module=editor&act=dispEditorPopup&editor_sequence=" +
    editor_sequence +
    "&component=" +
    component_name;
  if (typeof manual_url != "undefined" && manual_url)
    popup_url += "&manual_url=" + escape(manual_url);
  if (typeof current_mid != "undefined" && current_mid)
    popup_url += "&mid=" + escape(current_mid);
...
function editorSearchComponent(evt) {
  ...
  if (obj.getAttribute("widget")) {
    ...
    if (
      !tobj ||
      tobj.nodeName != "BODY" ||
      !tobj.getAttribute("editor_sequence")
    ) {
      editorPrevNode = null;
      return;
    }
    ...
    popopen(
      request_uri +
        "?module=widget&act=dispWidgetGenerateCodeInPage&selected_widget=" +
        widget +
        "&module_srl=" +
        editor_sequence,
      "GenerateCodeInPage",
...
function editorReplaceHTML(iframe_obj, html) {
  var range;
  var srcPathRegx =
    /src=("|\'){1}(\.\/)?(files\/attach|files\/cache|files\/faceOff|files\/member_extra_info|modules|common|widgets|widgetstyle|layouts|addons)\/([^"\']+)\.(jpg|jpeg|png|gif)("|\'){1}/g;
  ...
  var editor_sequence =
    iframe_obj.editor_sequence ||
    iframe_obj.contentWindow.document.body.getAttribute("editor_sequence");
  try {
    iframe_obj.contentWindow.focus();
  } catch (e) {}
  if (jQuery.isFunction(iframe_obj.replaceHTML)) {
    ...
    try {
      ...
      xInnerHtml(
        iframe_obj.contentWindow.document.body,
...
/* Original file: files/cache/assets/minified/modules.editor.skins.simpleeditor.js.simpleeditor.min.js */
...
(function ($) {
  ...
  var insertContent = function (instance, content) {
    ...
    if (String(navigator.userAgent).match(/Trident\/7/)) {
      ...
      document.execCommand("insertHTML", !1, content);
    }
  };
  var simplifyContent = function (str) {
    ...
    str = str.replace(/<!--(.*?)-->/gs, "");
    str = str.replace(
      /<\/?(\?xml|meta|link|font|span|style|script|noscript|frame|noframes|(?:st1|o):[a-z0-9]+)\b[^>]*?>/gi,
      "",
    );
    str = str.replace(
      /\b(id|class|style|on(?:[a-z0-9]+)|Mso(?:[a-z0-9]+))="[^"]*"/gi,
      "",
    );
  ...
  var convertYouTube = function (str) {
    var regexp =
      /(src=")?https?:\/\/(www\.youtube(?:-nocookie)?\.com\/(?:watch\?v=|v\/|embed\/)|youtu\.be\/)([a-zA-Z0-9_-]+)\S*/g;
    return String(str).replace(regexp, function (match, p1, p2, p3) {
      if (p1 === 'src="') {
        ...
        return (
          ...
          p3 +
          '" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe><p></p>'
        );
      }
    });
  };
...
/* Original file: files/cache/assets/minified/modules.editor.skins.simpleeditor.js.interface.min.js */
